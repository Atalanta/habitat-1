    ...
    clap_app!(("hab-sup") =>
        ...
        (@subcommand run =>
            ...
            (@arg PKG_IDENT_OR_ARTIFACT: +takes_value "Load the given Habitat package as part of \
                ...
                (ex: core/redis) or filepath to a Habitat Artifact \
                (ex: /home/core-redis-3.0.7-21120102031201-x86_64-linux.hart).")
            (@arg APPLICATION: --application -a +takes_value requires[PKG_IDENT_OR_ARTIFACT]
                requires[ENVIRONMENT] "Application name; [default: not set].")
            (@arg ENVIRONMENT: --environment -e +takes_value requires[PKG_IDENT_OR_ARTIFACT]
                requires[APPLICATION] "Environment name; [default: not set].")
            (@arg CHANNEL: --channel +takes_value requires[PKG_IDENT_OR_ARTIFACT]
                "Receive package updates from the specified release channel [default: stable]")
            (@arg GROUP: --group +takes_value requires[PKG_IDENT_OR_ARTIFACT]
                "The service group; shared config and topology [default: default].")
            (@arg BLDR_URL: --url -u +takes_value {valid_url} requires[PKG_IDENT_OR_ARTIFACT] 
                "Receive package updates from Builder at the specified URL \
                [default: https://bldr.habitat.sh]")
            (@arg TOPOLOGY: --topology -t +takes_value {valid_topology}
                requires[PKG_IDENT_OR_ARTIFACT] "Service topology; [default: none]")
            (@arg STRATEGY: --strategy -s +takes_value {valid_update_strategy}
                requires[PKG_IDENT_OR_ARTIFACT] 
                "The update strategy; [default: none] [values: none, at-once, rolling]")
            (@arg BIND: --bind +takes_value +multiple requires[PKG_IDENT_OR_ARTIFACT] 
                "One or more service groups to bind to a configuration")
            (@arg FORCE: --force -f requires[PKG_IDENT_OR_ARTIFACT]
                "Load or reload an already loaded service. If the service was previously loaded \
                and running this operation will also restart the service")
        )
        (@subcommand sh =>
            (about: "Start an interactive Bourne-like shell")
            (aliases: &[])
...
fn sub_run(m: &ArgMatches, launcher: LauncherCli) -> Result<()> {
    ...
    if Manager::is_running(&cfg)? {
        ...
        let svc = if let Some(pkg) = m.value_of("PKG_IDENT_OR_ARTIFACT") {
            // JW EOD: Install a package on the CLI side of things if we're running the Supervisor
            // and then provide an *exact* package ident to SvcLoad passed in here. Respect that
            // and exact ident was used and look on disk for the package.
            let mut msg = protocols::ctl::SvcLoad::new();
            update_svc_load_from_input(m, &mut msg)?;
