...
impl IntoServiceSpec for SvcLoad {
    ...
    fn into_composite_spec(
        ...
        // JW TODO: These type gymnastics need to go away. We need to store these types as
        // protobuf backed types instead of pure rust types.
        let mut composite_binds = self.get_composite_binds()
            .clone()
            .into_iter()
            .map(|(k, mut v)| {
                (
