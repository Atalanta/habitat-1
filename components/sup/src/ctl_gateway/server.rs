...
use protocols;
use protocols::net::{self, NetErr, NetResult};
...
impl CtlRequest {
        ...
        T: Into<SrvMessage> + fmt::Debug,
    {
        ...
        let mut wire: SrvMessage = msg.into();
...
impl UIWriter for CtlRequest {
    type ProgressBar = protocols::ctl::NetProgressBar;
...
impl Client {
    ...
    fn handshake(&self, socket: SrvStream) -> Box<Future<Item = SrvStream, Error = HandlerError>> {
        ...
        let handshake = socket
            ...
            .and_then(|(txn, io)| send_complete(io, txn, net::ok()));
                                ...
                                match msg.message_id() {
                                    "SvcGetDefaultCfg" => {
                                        ...
                                        CtlCommand::new(
                                            ...
                                            msg.transaction(),
                                            move |state, req| {
                                                Manager::service_cfg(state, req, m.clone())
                                            },
                                        )
                                    }
                                    "SvcLoad" => {
                                        ...
                                        CtlCommand::new(
                                            ...
                                            msg.transaction(),
                                            move |state, req| {
                                                Manager::service_load(state, req, m.clone())
                                            },
                                        )
                                    }
                                    "SvcUnload" => {
                                        let m = msg.parse::<protocols::ctl::SvcUnload>().unwrap();
                                        CtlCommand::new(
                                            Some(self.tx.clone()),
                                            msg.transaction(),
                                            move |state, req| {
                                    ...
                                    "SvcStart" => {
                                        ...
                                        CtlCommand::new(
                                            ...
                                            msg.transaction(),
                                            move |state, req| {
                                                Manager::service_start(state, req, m.clone())
                                            },
                                        )
                                    }
                                    "SvcStop" => {
                                        let m = msg.parse::<protocols::ctl::SvcStop>().unwrap();
                                        CtlCommand::new(
                                            Some(self.tx.clone()),
                                            msg.transaction(),
                                            move |state, req| {
                                                Manager::service_stop(state, req, m.clone())
                                            },
                                        )
                                    }
                                    "SvcStatus" => {
                                        let m = msg.parse::<protocols::ctl::SvcStatus>().unwrap();
                                        CtlCommand::new(
                                            Some(self.tx.clone()),
                                            msg.transaction(),
                                            move |state, req| {
...
where
    T: Into<SrvMessage> + fmt::Debug,
{
    let mut message: SrvMessage = reply.into();
