...
impl<'a> InstallTask<'a> {
    ...
    fn store_artifact_in_cache(
        ...
        let mut cache_path = self.cached_artifact_path(ident);
        // This call can fail to canonicalize paths under certain circumstances. This will allow
        // failure and continue to try if we were unable to canonicalize.
        if let Some(canonicalized) = cache_path.canonicalize().ok() {
            cache_path = canonicalized;
        }
